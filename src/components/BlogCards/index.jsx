...
export default function Blog(props) {
	...
	const isOwner =
		props.user._id === useSelector((state) => state.user._id)
			? true
			: false;
	...
	const handleBlogDelete = async () => {
		const deleteBlog = await axios.post(
			`${process.env.REACT_APP_API_URL}/blog/deleteblog`,
			{ id: props.id },
			...
			<CardContent style={{ position: "relative" }}>
				...
				<div
					...
					dangerouslySetInnerHTML={{
						__html: props.content.slice(0, 150) + "...",
					}}
				></div>
				<Button
				...
				<Typography
					...
					color="textSecondary"
				>
					{props.views} views &bull;{" "}
					{calculateAgeOfBlog(props.createdAt)} &bull;{" "}
					{props.impressed} Likes &nbsp;
					{isOwner ? (
						<>
							<IconButton
								aria-label="more"
